== Exercise: Tracing the lifecycle with notifications
...
Create two activities which extend this one. 
The first activity should allow to start the second one via an `Intent`.
...
image::activity_lifecycle20.png[Notifications about the lifecycle]
...
Validate that the selection of the spinner is saved and restored between configuration changes by the Android system.
